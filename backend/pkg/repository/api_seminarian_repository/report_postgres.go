...
type SeminarianReport struct {
	db *sqlx.DB
}

func NewSeminarianReport(db *sqlx.DB) *SeminarianReport {
	return &SeminarianReport{db: db}
}

func (r *SeminarianReport) GetThemesFromDiscipline(disciplineId int) ([]model.Section, error) {
...
func (r *SeminarianReport) GetAllTests(sectionId int) ([]model.TestsReport, error) {
...
func (r *SeminarianReport) GetAllLabs(sectionId int) ([]model.LabsReport, error) {
...
func (r *SeminarianReport) GetAllStudents(groupId int) ([]model.StudentReport, error) {
...
func (r *SeminarianReport) GetStudentMarkTest(studentId, testId int) int {
	var mark int
...
func (r *SeminarianReport) GetStudentMarkLab(studentId, labId int) int {
	var mark int
...
func (r *SeminarianReport) GetMarkFromSection(userId, sectionId int) (int, error) {
	mark := 0
...
func (r *SeminarianReport) GetSummaryCountSection(disciplineId int) int {
	var count int
...
func (r *SeminarianReport) GetMarkFromExam(userId, disciplineId int) int {
	var mark int
...
func (r *SeminarianReport) GetSummaryMarkFromSections(userId, disciplineId int) int {
...
func (r *SeminarianReport) GetAttendanceLesson(studentId, disciplineId int) int {
	var mark int
...
func (r *SeminarianReport) GetAttendanceSeminar(studentId, disciplineId int) int {
	var mark int
...
func (r *SeminarianReport) GetResult(studentId, disciplineId int) (string, string) {
	result := "зачтено"
	sections, err := r.GetThemesFromDiscipline(disciplineId)
	if err != nil {
		return "не зачтено", "F"
...
func (r *SeminarianReport) GetSectionsResult(studentId, disciplineId int) string {
	result := "а"
	sections, err := r.GetThemesFromDiscipline(disciplineId)
	if err != nil {
		return "н/а"
...
func (r *SeminarianReport) GetMarkFromTest(studentId, testId int) bool {
	var mark int
	query := fmt.Sprintf("SELECT mark FROM %s WHERE user_id = $1 AND test_id = $2", table_name.TestsMarkTable)
...
func (r *SeminarianReport) GetMarkFromLab(studentId, laboratoryId int) bool {
	var mark int
	query := fmt.Sprintf("SELECT mark FROM %s WHERE user_id = $1 AND laboratory_id = $2", table_name.LaboratoryMarkTable)
...
func (r *SeminarianReport) CheckAccessForGroup(seminarianId, groupId, disciplineId int) error {
	var groups []int
	errAccess := errors.New("access denied to group")
	query := fmt.Sprintf("SELECT group_id FROM %s WHERE user_id = $1 AND group_id = $2 AND discipline_id = $3", table_name.SeminariansGroupsTable)
	err := r.db.Select(&groups, query, seminarianId, groupId, disciplineId)
	if err != nil {
		return errAccess
	}
	if len(groups) == 0 {
