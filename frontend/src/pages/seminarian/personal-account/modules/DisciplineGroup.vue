<template>
  ...
  >
    <div
      class="test-modal"
      :style="{ minWidth: tab === 'close' ? '500px' : '950px' }"
    >
      <q-tabs
        v-model="tab"
        dense
...
const groups = computed(() => groupStore.commonGroups);
...
watch(groupId, async () => {
  if (!Array.isArray(groupId.value)) {
    await groupStore.getCommonGroups();
...
sectionStore.getSectionsSeminarian(Number(disciplineId.value));
...
onMounted(async () => {
  await groupStore.getSeminarianStudentsFromGroup(String(groupIdNumber.value));
});
...
onMounted(async () => {
  if (!Array.isArray(disciplineId.value) && !Array.isArray(groupId.value)) {
    ...
    await attendanceStore.getDisciplineLessonsSeminarian(
      Number(disciplineId.value)
    );
    await attendanceStore.getSeminarsSeminarian(
      Number(disciplineId.value),
      Number(groupId.value)
    );
    await attendanceStore.getGroupLessonsSeminarian(
      groupIdNumber.value,
      disciplineIdNumber.value
    );
  }
...
const createSeminar = async () => {
  ...
  await attendanceStore.createSeminarSeminarian({
...
const groupStudents = computed(() => groupStore.seminarianGroupStudnets);
