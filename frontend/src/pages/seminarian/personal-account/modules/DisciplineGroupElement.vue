<template>
  ...
  <q-dialog v-model="groupAttendanceModal">
    <div class="flex column g-m modal">
      <div class="flex justify-between text-primary att-header">
        <div>Студент</div>
...
import { ISeminar } from 'src/models/attendance/attendance';
...
const openGroupAttendanceModal = async () => {
  await store.getSeminarVisitingSeminarian(Number(props.seminar.seminar_id));
  if (groupStudents.value === null && allGroupStudents.value !== null) {
    await Promise.all(
      allGroupStudents.value.map(async (student) => {
        await store.addSeminarVisitingSeminarian(
          true,
...
const changeSeminar = async () => {
  ...
  await store.changeSeminarNameSeminarian(
    Number(props.seminar.seminar_id),
    newName.value,
    Math.floor(dateParse.getTime() / 1000)
  );

  await store.getSeminarsSeminarian(props.disciplineId, props.groupId);
...
const deleteSeminar = async () => {
  await store.deleteSeminarSeminarian(Number(props.seminar.seminar_id));
  await store.getSeminarsSeminarian(props.disciplineId, props.groupId);
};
</script>
